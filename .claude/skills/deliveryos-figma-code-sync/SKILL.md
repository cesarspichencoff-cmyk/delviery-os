...
O verificador que importa é executável, porque é o único que pega mapeamento
apontando para arquivo que não existe:

```bash
npm run test:platform:figma
```

Ele afirma que todo `code_path` do `COMPONENT_MAPPING.md` existe no repositório,
que todo estado crítico aparece no inventário, e que nenhuma tela `designed_future`
está sem marcação.
