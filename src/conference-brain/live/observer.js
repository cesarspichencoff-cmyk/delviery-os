...
function createLiveObserver(opts) {
   ...
   * Relogio injetavel.
   *
   * Antes disto, o ciclo chamava `new Date()` em cinco pontos, e o instante em
   * que uma observacao foi registrada dependia de quando o processo rodou. Duas
   * consequencias ruins: o teste precisava esperar tempo real para provar
   * envelhecimento, e dois replays do mesmo log produziam carimbos diferentes.
   *
   * Com o relogio como parametro, o tempo vira DADO — controlavel no teste e
   * deterministico no replay. O padrao continua sendo o relogio de parede, para
   * quem nao injeta nada nao mudar de comportamento.
   */
  const agora = typeof o.now === "function" ? o.now : () => new Date().toISOString();
  const runId = o.runId || (OBSERVER_VERSION + ":" + agora());
  ...
  async function runCycle() {
    ...
    try {
      ...
      const rec = {
        ...
        source_health: "unavailable", finished_at: agora(),
    ...
    if (requiresHumanIntervention(health.state)) {
      const rec = {
        run_id: runId, cycle_id: cycleId, started_at: startedAt,
        collector_version: o.collectorVersion || OBSERVER_VERSION,
        source_health: health.state, finished_at: agora(),
    ...
    for (const id of missing) {
      ...
      store.put("live_observations", Object.assign({}, last, {
        run_id: runId, cycle_id: cycleId, observed_at: agora(),
