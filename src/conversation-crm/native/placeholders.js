...
const CLASSIFICATIONS = deepFreeze({
  CONFIRMADO: ['{{LARGE_GROUP_POLICY}}','{{SIMULATED_DRIVER_CONFIG}}'],
  CONFIRMADO_COM_RESSALVA: ['{{TATA_UNITS}}','{{TATA_OPENING_HOURS}}','{{TATA_ADDRESS}}','{{HUMAN_SERVICE_CHANNEL}}','{{PAYMENT_METHODS}}','{{RESERVATION_POLICY}}','{{CORKAGE_FEE}}','{{COMPENSATION_POLICY}}','{{FOOD_SAFETY_TECHNICAL_OWNER}}','{{NOTIFICATION_POLICY}}'],
  CONFLITO: ['{{TATA_MENU_URL}}','{{WAITLIST_POLICY}}'],
  NAO_DEFINIDO: ['{{ANIMO_SYSTEM_IDENTITY}}','{{HUMAN_SERVICE_HOURS}}','{{GETIN_RESERVATION_URL}}','{{GETIN_WAITLIST_URL}}','{{DELIVERY_URL}}','{{OPERATIONAL_RESPONSE_TARGET}}','{{COMMERCIAL_OPERATIONAL_RESPONSE_TARGET}}','{{MANAGEMENT_RESPONSE_TARGET}}','{{FOOD_SAFETY_RESPONSE_TARGET}}','{{PRIVACY_RESPONSE_TARGET}}','{{REFUND_POLICY}}','{{REPLACEMENT_POLICY}}','{{LOCAL_HEALTH_SURVEILLANCE_CONTACT}}','{{HUMAN_HANDOFF_OWNERS}}','{{PRIVACY_NOTICE_URL}}','{{DATA_SUBJECT_CHANNEL}}','{{EVIDENCE_RETENTION_POLICY}}'],
  TECNICO_RECOMENDADO: ['{{STORE_UNIT_CONFIG}}','{{DELIVERYOS_DRIVER_FLAGS}}','{{DRIVER_READ_MODE}}','{{DRIVER_WRITE_MODE}}','{{DRIVER_TIMEOUT_MS}}','{{DRIVER_RETRY_POLICY}}','{{DRIVER_CIRCUIT_BREAKER}}','{{DRIVER_HEALTH_CHECK}}','{{IDEMPOTENCY_RETENTION_POLICY}}','{{CONVERSATION_NATIVE_FEATURE_FLAGS}}'],
...
function loadPlaceholderRegistry(catalogs = loadRuntimeCatalogs()) {
  ...
  const records = QUESTIONNAIRE_ORDER.map((key, index) => { const item=sourceByKey.get(key); if(!item)throw nativeError('PLACEHOLDER_REGISTRY_INCOMPLETE'); return deepFreeze({ ...item, item_number: index+1, preliminary_classification: classificationByKey.get(key), implementation_available: key === '{{SIMULATED_DRIVER_CONFIG}}', production_available: false }); });
  return deepFreeze({ records, byKey: new Map(records.map((item) => [item.key, item])), implementation_blockers_open: records.filter((item) => item.blocks_implementation && !item.implementation_available).length, production_blockers_open: records.filter((item) => item.blocks_production).length });
}
...
module.exports = { QUESTIONNAIRE_ORDER, CLASSIFICATIONS, loadPlaceholderRegistry, resolvePlaceholder };
