# Regra operacional confirmada — 2026-09-26

Fonte humana: César.

## Embalagem da Cozinha
- prato grande da Cozinha → **caixa 1.500**;
- qualquer item da Cozinha que não seja prato grande → **caixa 650 selada**.

Exemplos confirmados de 650 selada: Tempurá de milho, Ebi Spicy, Edamame e Guioza. A regra é geral e não se limita a esses exemplos.

## Sashimi
- 1 → **240**
- 2–4 → **450**
- 5–7 → **750**
- 8+ → **1.500**

## Nomenclatura das praças
Os códigos internos não mudam:
- `cozinha_quentes` = **Cozinha**
- `enrolados_quentes` = **Sushi Quentes**

São praças operacionais distintas.

## Catálogo
- Baunilha, Pistache e Melão são sabores do Choux Cream.
- A retirada dos menus sazonais do conjunto ativo está correta.
