...
const {scenarioInputFor}=require('../../src/conversation-crm/native/simulator');
...
test('chamadas concorrentes equivalentes mantêm um único efeito lógico',async(t)=>{const runtime=new NativeConversationRuntime({runtimeRoot:temp(t,'deliveryos-native-concurrent-')});const input=scenarioInputFor('TATA-SC-188',runtime.clock);const results=await Promise.all(Array.from({length:5},()=>Promise.resolve().then(()=>runtime.processMessage(input))));assert.equal(results.filter((item)=>item.duplicate===false).length,1);assert.equal(runtime.store.eventsOfType('action.completed').length,1);assert.equal(runtime.store.eventsOfType('driver_health.recorded').length,1);assert.equal(runtime.store.eventsOfType('runtime.response_registered').length,1);});
