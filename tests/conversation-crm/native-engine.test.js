...
test('as 51 intenções simuladas permanecem cobertas e Oke fica no catálogo operacional externo ao oráculo',()=>{const covered=new Set();for(const scenario of oracle.scenarios.scenarios){const result=engine.analyze({content:scenario.input,context:{}});assert.equal(result.intent,scenario.intent);assert.equal(result.scenario_id,null);assert.notEqual(result.intent,'reclamação');covered.add(result.intent);}assert.equal(covered.size,51);const operationalOnly=catalogs.intents.intents.map((item)=>item.id).filter((id)=>!covered.has(id));assert.deepEqual(operationalOnly,['event.oke_pickup']);});
...
test('registro carrega 47 placeholders, resolve fatos públicos e mantém fixture distinta',()=>{const registry=loadPlaceholderRegistry(catalogs);assert.equal(registry.records.length,47);assert.equal(registry.implementation_blockers_open,0);assert.equal(registry.production_blockers_open,32);assert.equal(registry.records[46].preliminary_classification,'CONFIRMADO');const confirmed=resolvePlaceholder(registry,'{{TATA_ADDRESS}}');assert.equal(confirmed.status,'confirmed');assert.equal(confirmed.institutional_fact,true);const fixture=resolvePlaceholder(registry,'{{TATA_ADDRESS}}',{synthetic:true,value:'ENDERECO_SINTETICO'});assert.equal(fixture.status,'synthetic_fixture');assert.equal(fixture.institutional_fact,false);});
